...
int main(int argc, char *argv[]) {
    try {
        ...
        while (true) {
            ...
            cv::putText(frame, out.str(), cv::Point2f(0, 45), cv::FONT_HERSHEY_TRIPLEX, 0.6, cv::Scalar(0, 255, 0));
            ...
            if (HeadPose.enabled() || AgeGender.enabled() || EmotionsDetection.enabled()) {
                ...
                cv::putText(frame, out.str(), cv::Point2f(0, 65), cv::FONT_HERSHEY_TRIPLEX, 0.6, cv::Scalar(0, 255, 0));
            }
            ...
            for (auto & result : FaceDetection.results) {
                ...
                auto genderColor = (AgeGender.enabled() && (i < AgeGender.maxBatch)) ?
                              ((AgeGender[i].maleProb < 0.75) ? cv::Scalar(147, 20, 255) : cv::Scalar(255, 0, 0)) :
